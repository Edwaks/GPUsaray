...
**GPUsaray** is a rental service for GPU computing capacity. It is designed for researchers, developers, and machine-learning teams and makes selecting and renting computing resources straightforward, with prices calculated in rubles.

- Public URL: https://gpusaray.com/ and https://my.gpusaray.com/
- Core entities: configurations, capacity, rental periods, orders, charges, and service states.
- Key capabilities: capacity catalog, configuration comparison, prices in rubles, personal account, orders and rental periods, service status control, billing history.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. GPUsaray provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Configurations, capacity, rental periods, orders, charges, and service states are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Visitor, Renter, and Budget owner work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Capacity catalog** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Configuration comparison** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Prices in rubles** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Personal account** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Orders and rental periods** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Service status control** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Billing history** — a distinct part of the workflow, connected to the product model and a clear user outcome.
...
### Gpusaray
![Gpusaray](docs/screenshots/gpusaray.png)

### Gpusaray full
![Gpusaray full](docs/screenshots/gpusaray_full.png)

### Gpusaray cabinet
![Gpusaray cabinet](docs/screenshots/gpusaray_cabinet.png)
